...
# sudo ip tuntap add name outtap mode tap
# ...
sudo ip link add outtap type veth peer name outtap2
sudo ip link add pgwtap type veth peer name pgwtap2
sudo ovs-vsctl add-br mybridge
sudo ovs-vsctl set bridge sdnadapter protocols=OpenFlow13 
sudo ovs-vsctl set bridge sdnadapter other-config:datapath-id=5 
sudo ovs-vsctl set-controller sdnadapter "tcp:127.0.0.1:6663"
sudo ovs-vsctl add-port sdnadapter pgwtap
sudo ovs-vsctl add-port sdnadapter outtap
sudo ifconfig sdnadapter 0.0.0.0 promisc up
sudo ifconfig outtap 0.0.0.0 promisc up
sudo ifconfig outtap2 0.0.0.0 promisc up
sudo ifconfig pgwtap 0.0.0.0 promisc up
# for example, if the we want the IP/HW MAC of pgwtap2 to be 10.1.2.4/00:00:00:00:10:04 
sudo ifconfig pgwtap2 10.1.2.4 netmask 255.255.255.0 hw ether 00:00:00:00:10:04 promisc up
`
...
class LteAdapterController13(app_manager.RyuApp):
    ...
    def handle_connection_in(self, ev):
        ...
        if str(in_port) in self.out_port_map.keys():
            out_port = self.out_port_map[str(in_port)]
        else:
            ...
            <>
                ...
                >
                    ...
                    <div key="script-code-panel" >
                        <CodeEditor code={script_code} language="shell" ></CodeEditor>
                    </div>
                    <div key="controller-code-panel" >
                        <CodeEditor code={controller_code} language="python"></CodeEditor>
                    </div>
                </ResponsiveReactLayout>
            </>
        );
